...
from modeling.erp import balance_monthly, create_erp_data, mirror_intercompany_expenses_to_products, estimate_costs_from_payroll
...
def create_mapping_between_all(generated_data: dict = None, company_name: str = None, save_to_csv: bool = True) -> dict:
    ...
    estimated_financials = estimate_costs_from_payroll(df_pay=df_payroll)
...
def create_all_erp_data(generated_mapped_data: dict, company_name: str, save_to_csv: bool = True) -> dict:
    ...
    df_erp_budget = budget.generate_monthly_budget(df_erp_all)
...
def create_reporting(generated_erp_data: dict, company_name: str, save_to_csv: bool = True) -> dict:
    ...
    Creates all ERP-related data by combining generated and mapped data.
    """
    from modeling.budget import generate_weekly_budget, generate_difference_analysis

    df_products = generated_erp_data["df_erp_products_full"]
    df_products["vendor_id"] = np.nan

    budget_weekly_4pct = generate_weekly_budget(df_products, price_growth=0.04)
    budget_weekly_8pct = generate_weekly_budget(df_products, price_growth=0.08)

    sample = pd.read_csv("data/inputdata/budget_comments.csv")

    df_comments_4 = generate_difference_analysis(df_actuals=df_products, df_budget=budget_weekly_4pct, df_comments=sample, budget_version="4% Price Growth")
    df_comments_8 = generate_difference_analysis(df_actuals=df_products, df_budget=budget_weekly_8pct, df_comments=sample, budget_version="8% Price Growth")

    df = pd.concat([df_comments_4, df_comments_8], axis=0, ignore_index=True)
    df.drop(columns=["year", "week"], inplace=True, errors='ignore')

    if save_to_csv:
        output_dir = f"data/outputdata/fact"
        os.makedirs(output_dir, exist_ok=True)
        df.to_csv(f"{output_dir}/fact_reporting.csv", index=False)
        print(f"✔ Reporting data saved and generated!")
